...
const Tarea = {
  ...
  async crear({ titulo, prioridad, puntos, es_clinica, sprint_id, proyecto_id, fecha_entrega }) {
    const [r] = await pool.query(
      `INSERT INTO tareas (titulo, prioridad, puntos, estatus, es_clinica, sprint_id, proyecto_id, fecha_entrega)
       VALUES (?, ?, ?, 'disponible', ?, ?, ?, ?)`,
      [titulo, prioridad, puntos, es_clinica ? 1 : 0, sprint_id || null, proyecto_id || null, fecha_entrega || null]
    );
  ...
  async actualizar(id, campos) {
    const { responsable_id, estatus, titulo, prioridad, puntos, es_clinica, sprint_id, fecha_entrega } = campos;
    const [r] = await pool.query(
      `UPDATE tareas SET
      ...
      [
        ...
        sprint_id ?? null, fecha_entrega ?? null, responsable_id ?? null, id
      ]
    );
